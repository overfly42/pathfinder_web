...
Reading a character (`GET /api/characters/{id}`, `backend/app/sheet.py`'s `build_character_sheet`) follows the same sequence every time, regardless of which rule elements are actually involved. This is the **target** design (decided 2026-08-10); today's actual code only partially matches it — see the note at the end of this section.
...
2. **Build the raw `CharacterContext`** — a single dataclass holding every piece of a character's own state a handler could plausibly need to compute its effect: raw ability scores, `skill_ranks: dict[BaseSkill.id, int]`, levels/classes, the composition ids from step 1 (feat/trait/granted-ability/active-effect ids), and equipped gear — straight off the `Character`/`CharacterLevel`/... rows, no rule logic yet. Replaces what used to be "load the character's own base data" as loose local variables threaded by hand between `sheet.py` functions; now one typed object every handler can be called with.
3. **Resolve every collected UUID exactly once, in one flat pass** — looked up against that rule element's handler (`rules/race_abilities.py`'s `HANDLERS`, a future `rules/class_abilities.py`'s `HANDLERS`, `rules/effects.py`'s `EFFECT_HANDLERS`, `rules/modifiers.py`'s `stack()` for equipped-gear bonuses, ...), every one of them called with the *same* raw `CharacterContext` from step 2, and always returning `list[Modifier]`, never a mutated character. No target is privileged and no handler family goes "first" — every handler that exists in this codebase decides its output from raw composition data alone (its own instance/level/chosen-sub-option, or raw skill ranks for something like Skill Focus's +3-vs-+6-at-10-ranks threshold, `CLAUDE.md`'s own example), never from another handler's *computed* result. An id with no handler (e.g. Darkvision, or a race's size trait today) just passes through as flavor text with no computed effect.
4. **Group every returned `Modifier`, by target, and `stack()` each target's pool** (`ModifierTarget.SCORE`/`AC`/`SPEED`/`SAVE_*`/`SKILL`) — one number per target, still detached from any base value.
5. **Apply each target's stacked value on top of its base formula** to get the final sheet numbers — 10 + Dex mod + `stack(AC modifiers)` for AC, the class save-progression value + ability mod + `stack(SAVE_* modifiers)` for a save, and so on. This is ordinary sequential arithmetic in `sheet.py`, nothing more: it computes `ability_mods` before the line that needs them to build the `saves` list, and total land speed before `jump_skill_bonus(total_land_speed)`, the same way any function computes an intermediate value before using it. That ordering is real, but it's local to `sheet.py`'s own formulas, not a rule the handler-calling contract in step 3 has to enforce — no handler anywhere currently reads another handler's stacked output as its own input, so step 3 doesn't need phases. (If a future handler ever does need a resolved ability modifier as an input to its *own* decision logic — e.g. a class ability whose bonus scales directly off the character's effective Wisdom modifier, rather than off raw levels/ranks — *that* handler would gain a dependency step 3 doesn't have today, and this section should be revisited then; not designed for speculatively now.)
...
**Today's actual state (2026-08-10) only partially matches this.** No `CharacterContext` dataclass exists yet — `sheet.py` still threads loose local variables (`effective_scores`, `ability_mods`, `character.effects`, ...) between its own functions by hand, the same shape this section used to describe. `rules/race_abilities.py`/`rules/speed.py`'s `HANDLERS` take zero arguments (fine so far — none of their current content is conditional on anything but "does the character have this ability"); `rules/effects.py`'s `EFFECT_HANDLERS` takes only that effect's own `list[CharacterEffect]` instances, not the full context (see `rules/effects.py:1-88`, `sheet.py`'s Entfesselter-Barbar-Kampfrausch wiring, 2026-08-09). Deliberately not batch-migrated: each handler family moves to the real `CharacterContext` signature the next time it's touched — e.g. whenever a handler that's actually conditional on raw composition data beyond its own instance (Skill Focus's skill ranks, or reading a character's other active effects — several of the class-ability effects in `todos.md`'s "Effekt-Handler-Inventar" will need this) gets written — not as a standalone refactor with no new behavior to show for it. Once every handler family is on this signature, `rules/effects.py`'s current rationale for staying a separate registry from `rules/handlers.py` ("the call signature differs") no longer holds, and the two should merge.
