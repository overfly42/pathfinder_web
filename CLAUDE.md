...
**As of 2026-09-13, the project is in a testing phase**: a small group of testers uses the real `frontend/` + `backend/` + database stack directly to create and play characters, so the database holds real, actively-used data — not just disposable fixtures/test rows. Treat it with the same care called out below for the dev database (see the destructive-script warning under Working Conventions).

The three root-level `pathfinder-*-mock.html` files are now frozen: they are no longer the reference design surface for new UI work and are not kept in sync with `frontend/` — they stay in the repo only to convey the original design intent. Plan new frontend work directly against `frontend/` (React + Vite + TypeScript), not against the mocks. See `roadmap.md` for what's been built (foundation + which lifecycle slices) versus what still reads fixtures.

Target architecture:
- Frontend: React (`frontend/`, Vite + TypeScript) — in progress; see `roadmap.md` for which lifecycle slices are built.
- Backend: FastAPI (`backend/`; owns character state/progression logic, exposes domain objects rather than raw rows, evaluates which actions/options are legal for a character's current state). Users/characters and the reference-data families already promoted to real tables (races, classes, skills) are backed by the database; the remaining reference data (feats, spells, items, effects, and remaining class content) still reads static JSON fixtures (`backend/app/fixtures/*.json`) by deliberate roadmap decision (see `roadmap.md`'s "Reference data stays in JSON fixtures" guiding decision). See `roadmap.md`/`todos.md` for per-endpoint status.
- Database: PostgreSQL, via `docker-compose.yml` (`podman-compose up -d`, since this machine has Podman rather than Docker). SQLAlchemy + Alembic are wired (`backend/app/db.py`, `backend/app/config.py`, `backend/app/models/`, `backend/alembic/`); real domain tables exist (users, characters, races, classes, skills, ...) and hold real tester data. Vector extension deliberately deferred until something needs it (e.g. compendium search).
...
- `pathfinder-mock.html`, `pathfinder-character-creation-mock.html`, `pathfinder-levelup-mock.html` — the original static UI mockups (character sheet / character creation / level-up flow). Each is a single self-contained file with inline `<style>` and `<script>` (no shared `app.js`/`styles.css`, only a Google Fonts CDN link). Frozen as of the 2026-09-13 testing-phase decision: kept only to convey the original design intent, not maintained or used as a reference for new `frontend/` work.
...
- `backup_db.sh` — dumps the dev Postgres database (`pg_dump` via `podman exec`, gzipped) to `db_backups/` and prunes to the 7 most recent. Run automatically by `dev.sh` right after the db container comes up, so every dev session starts with a snapshot of whatever was there before migrations/seeding touch it. Added after the 2026-09-07 incident below — the dev DB had no backups at all before this.
...
- **Never point an ad-hoc/debug script at `app.db.engine` or `app.db.SessionLocal` for anything destructive** (`Base.metadata.drop_all`, `TRUNCATE`, bulk deletes, schema resets). Those are the real dev database — there is no production/staging split in this project, so a mistake there is unrecoverable data loss, not a scoped accident. On 2026-09-07 a debug script did exactly this (`drop_all` + `create_all` against `app.db.engine` while reproducing a test failure) and wiped every user and character in the dev DB; only base/catalog data was re-seedable from fixtures. If a script needs to freely create/drop tables, build its own engine against an isolated database the way `backend/tests/conftest.py` does (a dedicated `<db>_test`-style database, or an ephemeral one) — never reuse the app's real engine for that. `backup_db.sh` (see Files) now provides a rolling safety net, but it's a mitigation, not a substitute for this rule.
